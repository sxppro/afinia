...
import { getYAxisDomain } from '@/lib/chart';
      ...
      >
        {categories.map((category, index) => (
          <LegendItem
            key={`item-${index}`}
            name={category}
            color={colors[index] || 'fill-gray-500'}
...
const BarChart = React.forwardRef<HTMLDivElement, BarChartProps>(
  (props, forwardedRef) => {
    ...
    const categoryColors = new Map(
      categories.map((cat, idx) => [cat, colors[idx] || 'fill-gray-500'])
    );
    ...
    return (
      <div
        ...
        {...other}
      >
        <ResponsiveContainer initialDimension={{ width: 700, height: 320 }}>
          <RechartsBarChart
            data={data}
            onClick={
              ...
              content={({ active, payload, label }) => {
                const cleanPayload: TooltipProps['payload'] = payload
                  ? payload.map((item: any) => ({
                      ...
                      color: categoryColors.get(item.dataKey) as string,
                      type: item.type,
